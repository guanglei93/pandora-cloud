...
- 自动配置：PandoraSecurityAutoConfiguration、PandoraWebSecurityAutoConfiguration
...
## 架构总览
Pandora Cloud的安全体系由“自动配置 + 过滤器 + 权限服务”三层构成，配合WebProperties的前缀能力，形成统一的鉴权与授权流程。
...
## 结论
SecurityProperties提供了Token传递、Mock调试、免登录与密码编码等关键安全配置；PandoraSecurityAutoConfiguration与PandoraWebSecurityAutoConfiguration分别负责基础安全组件与Web层授权规则；TokenAuthenticationFilter与SecurityFrameworkServiceImpl共同完成Token校验与权限判定。通过注解扫描、配置兜底与扩展点，形成灵活可控的安全体系。
